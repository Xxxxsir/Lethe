...
from backdoor_train import local_dataset, extract_alpaca_dataset, smart_tokenizer_and_embedding_resize, modify_text, proj_emotion_format, proj_sst_format, DEFAULT_PAD_TOKEN, MODIFY_SRC_STRATEGIES
...
def load_data(dataset_name, args):
    ...
    if dataset_name == 'sst2':
        ...
        full_dataset = full_dataset.map(proj_sst_format, remove_columns=['text', 'label'])
        return full_dataset
...
def main():
    ...
    #adapter_path = args.adapter_path#os.path.join(args.adapter_path, max(os.listdir(args.adapter_path),key=extract_number))
    ...
    # Start of main evaluation loop for different data types
    for data_name in all_data:
        ...
        for j in range(args.n_eval):
            ...
            # Start of batch generation loop
